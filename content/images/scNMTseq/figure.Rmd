...
gg_promoter_stage <- liger_plots_promoter[[2]] + labs(x = 'tSNE 1', y = 'tSNE 2', col = '') + scale_color_manual(values = col_palette, na.value="grey50") +  theme(legend.position=c(1,0), legend.justification=c(1,0))
gg_genebody_view <- liger_plots_genebody[[1]] + labs(x = 'tSNE 1', y = 'tSNE 2', col = '') + scale_color_discrete(labels = c(met = 'met_genebody', rna = 'rna'), na.value="grey50")  + theme(legend.position=c(1,.05), legend.justification=c(1,0))
gg_genebody_stage <- liger_plots_genebody[[2]] + labs(x = 'tSNE 1', y = 'tSNE 2', col = '') + scale_color_manual(values = col_palette) + theme(legend.position=c(1,0), legend.justification=c(1,0))
```
...
plot_pls <- function(pls_obj, stage, legend.title = 'Stage', comp = c(1,2), col_palette) {
  ...
  ggplot(variates) + 
    ...
    labs(x = axes_labels[1], y = axes_labels[2])
}
```
...
pls_no.impute
pls_impute <- plot_pls(pls_obj = mmspls.imputed, stage = mmspls$stage, col_palette = col_palette)
pls_impute
```
...
ggsave(filename = 'pls.pdf', plot = pls_no.impute)
ggsave(filename = 'pls-kmeans.pdf', plot = plot_grid(pls_tbl) + theme_void())
ggsave(filename = 'mosaic.pdf', plot = gg_mosaic, width = 10, height = 4)
ggsave(filename = 'liger-stage.pdf', plot = gg_genebody_stage, width = 7, height = 5.5)
ggsave(filename = 'liger-view.pdf', plot = gg_genebody_view, width = 7, height = 5.5)
```
