...
# source('grid.ftable.R')
# plot_grid(grid.ftable(df_data))
```
...
mytheme <- gridExtra::ttheme_default(
    core = list(padding = unit(c(2.5, 10.5), "mm"), bg_params = list(fill = 'white', col=NA)),
  colhead=list(fg_params=list(col="blue", fontface=2, fontsize=11)))
# 
# ttheme_minimal(
...
colnames(df_data)[5] <- c('Missing values')
df_data
df_data[,5] <- paste0(df_data[,5],'%')
tbl <- tableGrob(df_data, theme = mytheme, rows = NULL)
# tbl <- gtable::gtable_add_grob(tbl,
#         grobs = rectGrob(gp = gpar(fill = NA, lwd = 2)),
#         t = 2, b = nrow(tbl), l = 1, r = ncol(tbl))
gg_data <- plot_grid(tbl) +  theme(plot.background = element_rect(color = "black"))
...
```{r}
load('rna.sce.matching.RData')
# rna.sce.matching <- readRDS('rna.sce.matching.umap.rds')
# rna.sce <- readRDS('rna.sce.umap.rds')
```
...
plot_reducedDims <- function(sce, reducedDim = 'UMAP', 
  ...
  df$stage <- sce$stage
  ggplot(df, aes_string(axes[1], axes[2])) + geom_point(aes(col=stage), alpha = 0.75) +
...
```{r}
cow_rna_umap <- plot_reducedDims(rna.sce.matching, reducedDim = 'UMAP', col_palette = col_palette)
cow_rna_umap
# ggsave('umap-rna.matching.pdf', width = 4, height = 2.3)
```

```{r}
# gg_rna <- plot_reducedDims(rna.sce.matching, col_palette = col_palette)
# plot_reducedDims(rna.sce, col_palette = col_palette)
```
...
## ------------- View
liger_promoter_view <- liger_plots_promoter[[1]] + labs(x = 'tSNE 1', y = 'tSNE 2', col = '') + scale_color_discrete(labels = c(met = 'met_promoter', rna = 'rna')) + theme(legend.position=c(1,0.05), legend.justification=c(1,0))
    # guides(col = guide_legend(title = ''))
liger_promoter_stage <- liger_plots_promoter[[2]] + labs(x = 'tSNE 1', y = 'tSNE 2', col = '') + scale_color_manual(values = col_palette, na.value="grey50") +  theme(legend.position=c(1,0), legend.justification=c(1,0), na.value="grey50")
liger_genebody_view <- liger_plots_genebody[[1]] + labs(x = 'tSNE 1', y = 'tSNE 2', col = '') +
    scale_color_discrete(labels = c(met = 'met_genebody', rna = 'rna'),
                         na.value = "grey50")  + theme(
                           legend.text = element_text(face=2, size=8),
                           legend.position = c(0.95, .03),
                           legend.justification = c(0.8, 0),
                           legend.box.background = element_rect(colour = "black"),
                           legend.title=element_blank())
## ------------- Stage
liger_genebody_stage <- liger_plots_genebody[[2]] + 
  labs(x = 'tSNE 1', y = 'tSNE 2', col = '') + 
  scale_color_manual( values = col_palette, na.value="grey50") + 
  theme(
    legend.position=c(1,0.02), legend.justification=c(1,0),
    legend.text = element_text(face=2, size=8),
    legend.box.background = element_rect(colour = "black"), 
    legend.title = element_blank()
  )
cow_liger_genebody_stage <- liger_genebody_stage
cow_liger_genebody_stage
cow_liger_genebody_view <- liger_genebody_view
cow_liger_genebody_view
```
...
```{r}
load(url('https://github.com/arorarshi/scNMT_seq_MOSAIC/blob/master/data/cv.stats.stage.noimp.rmEmbryonicLineage.Rdata?raw=true'))
...
plot_pls <- function(pls_obj, stage, legend.title = 'Stage', comp = c(1,2), col_palette) {
  ...
  ggplot(variates) + 
    ...
    theme(legend.position=c(0.9,0.06), legend.justification=c(0.6,.15), 
...
df <- round(df, 2)
df$'Data view' <- rownames(df)
df <- df[,c(3,1:2)]
mytheme <- gridExtra::ttheme_default(
    core = list(padding = unit(c(2.5, 8.5), "mm"), bg_params = list(fill = 'white', col=NA)),
  colhead=list(fg_params=list(col="blue", fontface=2, fontsize=11)))
tbl <- tableGrob(df, theme = mytheme, rows = NULL)
# tbl <- gtable::gtable_add_grob(tbl,
#         grobs = rectGrob(gp = gpar(fill = NA, lwd = 2)),
#         t = 2, b = nrow(tbl), l = 1, r = ncol(tbl))
gg_pls_impute <- plot_grid(tbl)
cow_pls_accuracy <- gg_pls_impute
cow_pls_accuracy
```
...
# 
# ```{r}
# gg_accuracy <- plot_table(round(df, 2))
# gg_accuracy
...
```{r, fig.width=8}
# cowplot::plot_grid(cow_pls_impute)
# cowplot::plot_grid(cow_pls_accuracy)
# cowplot::plot_grid(cow_pls_impute, cow_pls_accuracy, rel_heights = c(7,1), nrow=1)
```
...
#   area(t = 1, l = 5)
# )
patch_liger_genebody_stage <- cow_liger_genebody_stage + 
  guides(color = guide_legend(override.aes = list(size=1),
                              keywidth = 0.1,
                              keyheight=0.1,
                              default.unit="inch")) +
  theme(legend.text = element_text(face=2, size=6), legend.spacing.x = unit(0, 'mm'))

patch_liger_genebody_view <- cow_liger_genebody_view + 
  guides(color = guide_legend(override.aes = list(size=3),
                              keywidth = 0.1,
                              keyheight=0.1,
                              default.unit="inch")) + 
   theme(legend.text = element_text(face=2, size=5))

(plot_spacer()|cow_rna_umap)/
  (cow_mosaic)/
  (patch_liger_genebody_stage + void + patch_liger_genebody_view + plot_layout(widths = c(4,1,4))) /
  (cow_pls_impute) + plot_layout()
```
...
```{r}
ggsave(filename = 'data-overview.pdf', plot = cow_data, height=3)
ggsave(filename = 'rna-umap.pdf', plot = cow_rna_umap, width = 5, height = 4)
ggsave(filename = 'pls-impute.pdf', plot = cow_pls_impute + theme(legend.position = 'none'),
       # + theme(legend.position = 'none')
       , width = 8, height = 5)
# ggsave(filename = 'pls-no_impute.pdf', plot = pls_no.impute + theme(legend.title = element_blank()), width = 5, height = 5)
ggsave(filename = 'pls-kmeans.pdf', plot = cow_pls_accuracy)
ggsave(filename = 'mosaic.pdf', plot = cow_mosaic, width = 6, height = 2.4)
ggsave(filename = 'liger-stage.pdf', plot = cow_liger_genebody_stage, width = 5, height = 4)
ggsave(filename = 'liger-view.pdf', plot = cow_liger_genebody_view,  width = 5, height = 4)
```
